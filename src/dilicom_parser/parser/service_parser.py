...
class ServiceParser:
    ...
    Parseur pour les fichiers de messages de service (ATE/ALE/ANE/AST).
     - Le parser détecte automatiquement le format du fichier (GENCOD ou EANCOM) et utilise le parseur approprié.
    ...
    def parse_file(self, filename: str) -> Union[List[GencodServiceMessage], EancomInterchange]:
        ...
        if file_format == "eancom":
            ...
            logger.debug("Fichier '%s' parsé avec succès. Interchange EANCOM extrait.", filename)
            return self.eancom_interchange

        lines = content.splitlines()
        self.gencod_messages = parse_gencod_lines(lines)
        logger.debug("Fichier '%s' parsé avec succès. Nombre de messages GENCOD extraits: %d",
                     filename, len(self.gencod_messages))
    ...
    def parse_content(self, content: str) -> Union[List[GencodServiceMessage], EancomInterchange]:
        ...
        lines = content.splitlines()
        self.gencod_messages = parse_gencod_lines(lines)
        logger.debug("Contenu parsé avec succès. Nombre de messages GENCOD extraits: %d",
