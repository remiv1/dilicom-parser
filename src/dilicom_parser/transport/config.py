...
class DilicomConfig:
    ...
    def __repr__(self) -> str:
        return (
            f"<DilicomConfig, host:port={self.host}:{self.port}, "
...
def load_dilicom_config(env_path: Optional[str] = None) -> DilicomConfig:
    ...
    logger.debug(
        "Chargement de la configuration Dilicom à partir des variables d'environnement."
    )
    ...
    logger.debug("Configuration Dilicom : %r", config)
    return config
